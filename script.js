    ...
    fetch('questions.json')
        ...
        .then(data => {
            ...
            if (!Array.isArray(questions) || questions.length === 0) {
                throw new Error('Dữ liệu câu hỏi không hợp lệ hoặc rỗng');
            }
        ...
        .catch(error => {
            ...
            alert('Không thể tải câu hỏi. Vui lòng kiểm tra tệp questions.json hoặc kết nối mạng.');
        });
});
...
function startQuiz() {
    ...
    if (!numQuestions || numQuestions > questions.length) {
